...
DISCRIMINATOR_MODELS_PARAMS = {
    ...
    # },
    "Strategy": {
        "path": "../StrategyClassifier_head/output_blenderbot/ESC_classifier_head_epoch_10.pt",
        "class_size": 8,
        "embed_size": 512,
        "pretrained_model": "../GenerationModel/DATA/strat_pp.strat/2025-10-03012322.3e-05.16.1gpu/epoch-2.bin",
        "class_vocab": {"Others": 0, "Self-disclosure": 1, "Affirmation and Reassurance": 2, "Providing Suggestions": 3,
                        "Reflection of feelings": 4, "Information": 5, "Restatement or Paraphrasing": 6, "Question": 7}
    },
    "Emotion_GO": {
        "path": "../EmotionClassifier_head/output_blenderbot/GO_classifier_head_epoch_10.pt",
        "class_size": 28,
        "embed_size": 512,
        "pretrained_model": "../GenerationModel/DATA/strat_pp.strat/2025-10-03012322.3e-05.16.1gpu/epoch-2.bin",
        "class_vocab": {"caring": 0, "gratitude": 1, "love": 2, "pride": 3, "approval": 4, "disgust": 5,
                        "optimism": 6, "surprise": 7, "fear": 8, "realization": 9, "excitement": 10,
                        "disappointment": 11, "grief": 12, "confusion": 13, "sadness": 14, "amusement": 15,
                        "embarrassment": 16, "joy": 17, "nervousness": 18, "relief": 19, "annoyance": 20,
                        "curiosity": 21, "neutral": 22, "desire": 23, "disapproval": 24, "anger": 25,
                        "admiration": 26, "remorse": 27}
    }
...
def RSA_inference_blenderbot(
    ...
    beta = 0.9   # soften S0
    alpha = 6 # listener contrast strength
    lambda_rsa = 1
    ...
    w0 = 0.5
    lp_exp = listener_posterior.exp()  # [1, vocab, world_num]
    listener_score_combined = w0 * lp_exp[:, :, 0] + (1 - w0) * lp_exp[:, :, 1:].mean(dim=2)  # sum over other worlds
    listener_score = torch.log(listener_score_combined + 1e-12) 
...
def compute_window_mask(past, window_length, decay, device):
    _, _, curr_length, _ = past[0][0].shape
    if window_length == 0 or curr_length <= window_length:
    ...
    decay_mask = torch.arange(0., 1.0 + SMALL_CONST, 1.0 / window_length)[1:] if decay else 1.0
    ones = torch.ones(ones_shape).to(device)
    if isinstance(decay_mask, torch.Tensor):
        ones *= decay_mask.view(1, 1, -1, 1)
    window_mask = torch.cat((ones, torch.zeros(zeros_shape).to(device)), dim=-2)
    return window_mask
...
def classifying_attribute(encoder_outputs, encoder_attention_mask, curr_unpert_past, curr_probs, curr_length, model, wte, horizon_length, new_accumulated_hidden, classifier):
    delta = 0.7 # decay factor
    
    for _ in range(horizon_length):
        inputs_embeds = torch.matmul(curr_probs, wte.weight.data)
        
        curr_output = model.model.decoder(inputs_embeds=inputs_embeds,
                                    encoder_hidden_states=encoder_outputs[0],
...
def perturb_hidden(
    ...
    for i in range(num_iterations):
        ...
        curr_output = model.model.decoder(input_ids=last,
                                        encoder_hidden_states=encoder_outputs[0],
                                        encoder_attention_mask=encoder_attention_mask,
                                        past_key_values=perturbed_past,
                                        output_hidden_states=True,
                                        return_dict=True)
        all_logits = model.lm_head(curr_output.last_hidden_state) + model.final_logits_bias
        hidden = curr_output.last_hidden_state 
        ...
        if loss_type == LossType.ENGAGEMENT: # TODO, not included for ALL fow now.
            #system 
            classification = classifying_attribute(encoder_outputs, encoder_attention_mask,
                                                   curr_unpert_past,
...
def full_text_generation(
    ...
    if verbosity_level > REGULAR or (emo_w ==0 and str_w ==0):
...
def generate_text_pplm(
    ...
    for i in range_func:
        ...
        unpert_last_hidden, unpert_past, _ = model.model.decoder(
                                                    input_ids=decoder_input_ids,
                                                    encoder_hidden_states=encoder_outputs[0],
                                                    encoder_attention_mask=attention_mask,
                                                    output_hidden_states=True,
        ...
        unpert_logits = model.lm_head(unpert_last_hidden) + model.final_logits_bias
        ...
        if rsa and perturb:    
            ...
            pert_probs, new_worldpriors = RSA_inference_blenderbot(log_score, worldpriors, bad_words_mask, top_k, top_p)
            worldpriors = new_worldpriors
...
def run_pplm_example(
    ...
    if attribute_type == 'all' and (emo_weight > 0 or str_weight > 0):
        ...
        emo_attribute_flag = True
    elif num_iterations == 0 or (emo_weight == 0 and str_weight == 0):
    ...
    for batch, posts, references, sample_ids, histories, speaker_roles in infer_dataloader:   
        ...
        if page:
            ...
            # user_emotional_state, emo_w, str_w
            # breakpoint()

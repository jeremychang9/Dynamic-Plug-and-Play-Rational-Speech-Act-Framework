...
def build_model(only_toker=False, checkpoint=None, local_rank=-1, **kwargs):
    ...
    if hasattr(model, "final_logits_bias") and model.final_logits_bias is not None:
        if model.final_logits_bias.size(0) != len(toker):
            model.final_logits_bias = torch.nn.Parameter(
                torch.zeros(1, len(toker), device=model.final_logits_bias.device)
            )
    
    if checkpoint is not None:
        if local_rank == -1 or get_rank() == 0:
            logger.info('loading finetuned model from %s' % checkpoint)
        model.load_state_dict(torch.load(checkpoint, map_location=torch.device('cpu'), weights_only=True))
...
def load_model(model, checkpoint, local_rank=-1):
    if checkpoint is not None and checkpoint.lower() != "none":
        ...
        model_state_dict = torch.load(checkpoint, weights_only=True)
